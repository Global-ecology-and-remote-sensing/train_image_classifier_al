...
- Renamed save_checkpoint and load_checkpoint functions
    to save_embedding_model and load_embedding_model
    respectively
- Added code to end of getCriterion to record the loss_type,
    strategy and margin to make it easier to make checkpoints
"""
...
def save_embedding_model(state, is_best, filename='checkpoint.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'model_best.pth.tar')

def load_embedding_model(filename='model_best.pth.tar'):
...
def getCriterion(loss_type, strategy, margin):
    """
    Returns selected loss function for training
    a model
    
    Attributes
      - loss_type : string containing the name
        of the type of loss function
      - strategy : strategy for the chosen type
        of loss function
      - margin : Hyperparameter for siamese and
    ...
    if loss_type.lower() == 'softmax':
        ...
        criterion = OnlineTripletLoss(margin, selector)
    
    
    criterion.loss_type = loss_type
